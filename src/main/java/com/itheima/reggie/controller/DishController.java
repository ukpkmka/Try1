...
public class DishController {
    ...
    public R<String> change(HttpServletRequest request, @PathVariable Integer p) {
        log.info(request.toString());
        // 获取请求参数中的 ids，并将其转换为 List<Long>
        String idsParam = request.getParameter("ids");
        List<Long> ids = Arrays.stream(idsParam.split(","))
                .map(Long::parseLong)
                .collect(Collectors.toList());
        // 获取所有的 Dish 对象并设置状态
        List<Dish> dishesToUpdate = dishService.listByIds(ids).stream()
                .peek(dish -> dish.setStatus(p))
                .collect(Collectors.toList());
        // 批量更新
        dishService.updateBatchById(dishesToUpdate);
...
//    }
    ...
    public R<List<DishDto>> list(Dish dish) {
        ...
        List<DishDto> list2 = list.stream().map((item)->{
            ...
            List<DishFlavor> listsp = dishFlavorService.list(queryWrapper2);
            dishDto.setFlavors(listsp);
